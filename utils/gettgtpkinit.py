...
class myPKINIT(PKINIT):
    ...
    def decrypt_asrep(self, as_rep):
        ...
        enc_key = binascii.hexlify(t_key).decode('utf-8')
        logger.info(enc_key)
        ...
        return encasrep, session_key, cipher, enc_key

def amain(args,cert_pass,cert_pfx,domain,username):
    ...
    ini = myPKINIT.from_pfx(cert_pfx, cert_pass, dhparams)
    ...
    encasrep, session_key, cipher, enc_key = ini.decrypt_asrep(res.native)
    ccache = CCACHE()
